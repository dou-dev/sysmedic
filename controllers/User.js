...
export class userController {
  ...
  static async create (req, res) {
    try {
      ...
      if (userExists.success) return res.status(409).json({ message: userExists.message })

      const isCreatedUser = await userModel.create({ data: req.body })

      if (!isCreatedUser.success) return res.status(400).json(isCreatedUser.message)
  ...
  static async confirmAcount (req, res) {
    try {
      ...
      if (isConfirmed.success) res.status(200).json({ message: 'Email confirmed' })
    } catch (err) {
      console.error(err)
    }
  }
