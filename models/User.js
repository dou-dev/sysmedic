...
export class userModel {
  ...
  static async create({ data }) {
    ...
    return connection
      .query(
        "INSERT INTO user (first_name, last_name, email, password, token) VALUES (? , ?, ? , ?, ?)",
        [name, lastName, email, hashedPassword, token]
      )
      .then(([result, fields]) => {
  ...
  static async confirmAcount({ token }) {
    return connection
      .query('UPDATE user SET token = "", is_confirmed = 1 WHERE token =  ?', [
        token,
      ])
      .then(([result, fields]) => {
        return result.affectedRows < 1
